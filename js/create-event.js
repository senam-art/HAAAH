...
if (window.CREATE_EVENT_SCRIPT_LOADED) {
    console.warn("⚠️ create-event.js attempted to load a second time. Skipped.");
} else {
    window.CREATE_EVENT_SCRIPT_LOADED = true;
    ...
    window.initVenueMap = function() {
        ...
        if (!mapEl) {
            // Map element missing is okay on some pages, just return
            return;
        }
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if(payBtn) {
            // FIX: Remove existing listeners to be safe (cloning node trick)
            const newPayBtn = payBtn.cloneNode(true);
            payBtn.parentNode.replaceChild(newPayBtn, payBtn);
            newPayBtn.addEventListener('click', payWithPaystack);
        }
    });
    ...
    function updateMapMarker(venue) {
        if (!STATE.map) return;
        if (isNaN(venue.latitude) || isNaN(venue.longitude)) return;
    ...
    function calculateFinancials() {
        ...
        if(commDisplay && venueRate > 0) {
            if (totalRevenue >= totalNeeded) commDisplay.innerHTML = `Success! Profit: <span class="text-brand-accent font-bold">GHS ${(totalRevenue - totalNeeded).toFixed(2)}</span>`;
            else commDisplay.innerHTML = `Warning: <span class="text-red-400 font-bold">GHS ${Math.abs(totalRevenue - totalNeeded).toFixed(2)} loss</span>`;
        }
    }
    ...
    function initListeners() {
        ...
        if (form) {
            // FIX: Use onsubmit property to prevent duplicate listeners
    ...
    function payWithPaystack() {
        ...
        const handler = PaystackPop.setup({
            ...
            email: email,
            amount: STATE.pendingAmount * 100, 
            currency: 'GHS',
            ...
            metadata: {
                custom_fields: [
                    ...
                    { display_name: "Payment Type", variable_name: "payment_type", value: "organizer_fee" }
                ]
            },
    ...
    function handleFormSubmit(e) {
        e.preventDefault();
        e.stopImmediatePropagation(); // FIX: Stop other listeners from firing
        ...
        const submitBtn = document.getElementById(DOM.btn.submit);
        if (submitBtn.disabled) return; // Stop if already clicked
